...
const getTokenOnCode = (username, cb) => {

    const code = rl.question(`[bambulab.com] gmail verify code: `);
    const u = parse(BASE);
    u.method = 'POST';
    const json = JSON.stringify({ account: username, code: code, apiError: "" });
    logger.debug(`Payload:`, json);
    u.headers = {
        'content-type':   'application/json',
        'content-length': json.length
    };
    logger.debug(`getTokens with code`, u, code);
...
const getTokens = (username, passwd, cb) => {
    ...
    const req = https.request(u, (res) => {
        ...
        res.on('end', () => {
            ...
            if (json.loginType && json.loginType == "verifyCode") {
                getTokenOnCode(username, json.loginType, cb);
            } else {
                cb(null, tokens);
            }
        });
